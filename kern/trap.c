...
void aloc_func(struct Env * curenv, uint32 fault_va);
void place(struct Env * curenv, uint32 fault_va);
void replace(struct Env * curenv, uint32 fault_va);
void shift(struct Env * curenv);
...
void page_fault_handler(struct Env * curenv, uint32 fault_va) {
	int size_total = LIST_SIZE(&curenv->PageWorkingSetList);
	//cprintf("%d\n",size_total);
	//if law fe 7aga fe al page w s
	fault_va=ROUNDDOWN(fault_va,PAGE_SIZE);
	if (size_total != 0) {
		place(curenv,  fault_va);
	}
	// kda mfesh mkan fady fy l two lists
	else if (size_total == 0) {
		replace(curenv,  fault_va);
	}
}
void place(struct Env * curenv, uint32 fault_va){
	// else if law al active full bs second not full
	if (curenv->ActiveListSize - curenv->ActiveList.size == 0&& curenv->SecondListSize > curenv->SecondList.size) {
		shift(curenv);
		int flag = 0;
		...
		if (flag == 0) {
			aloc_func(curenv, fault_va);
		}
	}
	//if law al active not full
	else if (curenv->ActiveListSize > curenv->ActiveList.size) {
		aloc_func(curenv,fault_va);
	}
}
void replace(struct Env * curenv, uint32 fault_va){
	// else el active w second full
		int flag_1 = 0;
		shift(curenv);
			struct WorkingSetElement *found_in_second;
			LIST_FOREACH(found_in_second, &(curenv->SecondList))
		...
		if (flag_1 == 0) {
		struct WorkingSetElement* last_in_second = LIST_LAST(&curenv->SecondList);
		uint32 page_permissions=pt_get_page_permissions(curenv, last_in_second->virtual_address );
		// if modified
		if((page_permissions) & (PERM_MODIFIED)){
			uint32*p_t=NULL;
			struct Frame_Info* f_i=get_frame_info(curenv->env_page_directory,(void*)(last_in_second->virtual_address),&p_t);
			int r = pf_update_env_page(curenv,(void*)(last_in_second->virtual_address),f_i);
		}
		unmap_frame(curenv->env_page_directory,(void*)last_in_second->virtual_address);
		LIST_REMOVE(&curenv->SecondList, last_in_second);
		LIST_INSERT_HEAD(&curenv->PageWorkingSetList,last_in_second);
		aloc_func(curenv,fault_va);
	 }
}
void aloc_func(struct Env * curenv, uint32 fault_va ) {
	...
	uint32 *ptr_p_t = NULL;
	struct Frame_Info*p_a = get_frame_info(curenv->env_page_directory,(void*) fault_va, &ptr_p_t);
	int ret = allocate_frame(&p_a);
	if (ret != E_NO_MEM) {
		map_frame(curenv->env_page_directory, p_a, (void*) fault_va,PERM_WRITEABLE | PERM_PRESENT | PERM_USER);
	}
	struct WorkingSetElement* first_ele = LIST_FIRST(&(curenv->PageWorkingSetList));
	first_ele->virtual_address = fault_va;
	LIST_REMOVE(&curenv->PageWorkingSetList, first_ele);
	LIST_INSERT_HEAD(&curenv->ActiveList, first_ele);
	int ret0 = pf_read_env_page(curenv, (void*) fault_va);
	if (ret0 == E_PAGE_NOT_EXIST_IN_PF) {
		if (fault_va < USTACKTOP && fault_va > USTACKBOTTOM) {
			int ret1 = pf_add_empty_env_page(curenv, fault_va, 1);
			if (ret1 == E_NO_PAGE_FILE_SPACE) {
...
void shift(struct Env * curenv){
	struct WorkingSetElement* last_in_active = LIST_LAST(&curenv->ActiveList);
	LIST_REMOVE(&curenv->ActiveList, last_in_active);
	pt_set_page_permissions(curenv, last_in_active->virtual_address, 0,PERM_PRESENT);
	LIST_INSERT_HEAD(&curenv->SecondList, last_in_active);
}
void __page_fault_handler_with_buffering(struct Env * curenv, uint32 fault_va) {
