...
void free_from_active(struct Env* e, uint32 virtual_address, uint32 size);
void free_from_second(struct Env* e, uint32 virtual_address, uint32 size);
void for_free_from_pd(uint32 *return_page_table,
		struct WorkingSetElement*ele_is_found, struct Env* e);
...
void freeMem(struct Env* e, uint32 virtual_address, uint32 size) {
	int number_of_frames_to_free_pf = size / PAGE_SIZE;
	int counter = 0;
	while (counter < number_of_frames_to_free_pf) {
		uint32 va_to_free = virtual_address + (counter * PAGE_SIZE);
		pf_remove_env_page(e, (va_to_free));
		counter++;
	}
	free_from_active(e, virtual_address, size);
	free_from_second(e, virtual_address, size);
}
void free_from_active(struct Env* e, uint32 virtual_address, uint32 size) {
	uint32 va_rounded_up_to_size = (virtual_address + size);
	uint32 va_rounded_down = ROUNDDOWN(virtual_address, PAGE_SIZE);
	struct WorkingSetElement*ele_is_found_in_active;
	LIST_FOREACH(ele_is_found_in_active,&(e->ActiveList))
	{
		if (ele_is_found_in_active->virtual_address >= va_rounded_down && ele_is_found_in_active->virtual_address
						< va_rounded_up_to_size) {
			uint32*return_page_table = NULL;
			get_frame_info(e->env_page_directory,
					(void*) ele_is_found_in_active->virtual_address,
					&return_page_table);
			unmap_frame(e->env_page_directory,
					(void*) ele_is_found_in_active->virtual_address);
			LIST_REMOVE(&(e->ActiveList), ele_is_found_in_active);
			ele_is_found_in_active->empty = 1;
			LIST_INSERT_HEAD(&(e->PageWorkingSetList), ele_is_found_in_active);
			...
			//pt_clear_page_table_entry(e , ele_is_found_in_active->virtual_address);
			// function e7na 3malalha implement
			for_free_from_pd(return_page_table, ele_is_found_in_active, e);
		}
...
void free_from_second(struct Env* e, uint32 virtual_address, uint32 size) {
	uint32 va_rounded_up_to_size = (virtual_address + size);
	uint32 va_rounded_down = ROUNDDOWN(virtual_address, PAGE_SIZE);
	struct WorkingSetElement*ele_is_found_in_second;
	LIST_FOREACH(ele_is_found_in_second,&(e->SecondList))
	{
		uint32 va = ele_is_found_in_second->virtual_address;
		if (ele_is_found_in_second->virtual_address >= va_rounded_down
				&& ele_is_found_in_second->virtual_address
						< va_rounded_up_to_size) {
			uint32*return_page_table = NULL;
			struct Frame_Info*f = get_frame_info(e->env_page_directory,
					(void*) ele_is_found_in_second->virtual_address,
					&return_page_table);
			unmap_frame(e->env_page_directory, (void*) virtual_address);
			LIST_REMOVE(&(e->SecondList), ele_is_found_in_second);
			ele_is_found_in_second->empty = 1;
			LIST_INSERT_HEAD(&(e->PageWorkingSetList), ele_is_found_in_second);
			// function e7na 3malalha implement
			for_free_from_pd(return_page_table, ele_is_found_in_second, e);
		}
	}
}
void for_free_from_pd(uint32 *return_page_table,
	...
	for (int j = 0; j < 1024; j++) {
		if (return_page_table[j] != 0) {
			return;
		}
	}
	free_frame(to_frame_info((uint32) return_page_table & 0xffff000));
	pd_clear_page_dir_entry(e, ele_is_found->virtual_address);
	pd_set_table_unused(e, ele_is_found->virtual_address);
}
